...
func UInt64ToBytesArr(uint64Array []uint64) []byte {
	byteArr := make([]byte, 8*len(uint64Array))
	for i, uint64Value := range uint64Array {
		bIndex := i * 8
		binary.LittleEndian.PutUint64(byteArr[bIndex:bIndex+8], uint64Value)
	}
	return byteArr
}
